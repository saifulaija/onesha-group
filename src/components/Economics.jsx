...
/* ---------- one bar + its own tooltip, anchored to its own top ---------- */
const Bar = ({ bar, index, revealed, active, onActivate, onDeactivate }) => {
  const color = bar.belowBreakeven ? "#C2543F" : "#D97B29";
  ...
  return (
    <div
      ...
      onClick={onActivate}
    >
      {/* wrapper's own height == bar height, so the tooltip anchored to its
          top edge always sits exactly above the visible bar, at any height */}
      <div
        className="relative flex w-full justify-center transition-[height] duration-[1000ms] ease-out"
        style={{
          height: revealed ? `${bar.height}%` : "0%",
      ...
      >
        <div
          ...
          style={{
            background: `linear-gradient(180deg, ${color} 0%, ${color}CC 100%)`,
            boxShadow: active
              ? `0 0 0 1px ${color}, 0 10px 26px -8px ${color}99`
              : "none",
            filter: active ? "brightness(1.15)" : "brightness(1)",
          }}
        />

        {/* tooltip — anchored to the bar's own top, always visible regardless of height */}
        <div
          className="pointer-events-none absolute bottom-full z-20 mb-2.5 flex flex-col items-center transition-all duration-200 ease-out"
          style={{
            opacity: active ? 1 : 0,
            transform: active
        ...
        >
          <span
            className="whitespace-nowrap rounded-md px-2.5 py-1 text-[11px] font-semibold text-white shadow-[0_10px_24px_-8px_rgba(0,0,0,0.6)] sm:text-[12px]"
            style={{
          ...
          <span
            className="-mt-[1px] h-2 w-2 rotate-45"
      ...
      <span
        className="mt-2 text-[10px] text-[#B9C2CC] transition-colors duration-200 sm:text-[11px]"
        style={{
          fontFamily: "'JetBrains Mono', monospace",
          color: active ? "#fff" : "#B9C2CC",
        }}
      >
        {bar.label}
      </span>
...
const BreakevenChart = () => {
  ...
  return (
    ...
    >
      ...
      <p className="mb-2 text-[12px] text-[#B9C2CC] sm:text-[12.5px]">
        Containers shipped vs. cumulative margin, illustrative model
      </p>
      <p
        className="mb-8 text-[10.5px] text-white/30 sm:mb-10 sm:text-[11px]"
        style={{ fontFamily: "'JetBrains Mono', monospace" }}
      >
        Hover or tap a bar for its exact value
      </p>

      {/* extra top padding reserves room for the tallest bar's tooltip */}
      <div className="relative h-[240px] pt-10 sm:h-[280px] sm:pt-12">
        {/* gridlines */}
        <div className="pointer-events-none absolute inset-x-0 bottom-0 top-10 flex flex-col justify-between sm:top-12">
          {[100, 75, 50, 25, 0].map((g) => (
            <div key={g} className="flex items-center gap-2">
              <span
                className="w-6 shrink-0 text-right text-[9px] text-white/25 sm:w-8 sm:text-[10px]"
        ...
        {/* bars */}
        <div className="absolute inset-x-0 bottom-0 top-10 flex items-end gap-2.5 pl-8 sm:top-12 sm:gap-4 sm:pl-10">
          {BARS.map((bar, i) => (
            <Bar
              key={bar.label}
              bar={bar}
